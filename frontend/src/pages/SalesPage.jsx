...
const EMPTY_LINE = { product_id: "", product_label: "", quantity: "", unit_id: "", warehouse_id: "" };
...
function linesFromInvoice(invoice, products) {
  ...
  for (const line of invoice.lines) {
    ...
    if (!byProduct[key]) byProduct[key] = { quantity: 0, warehouse_id: line.warehouse_id };
    byProduct[key].quantity += Number(line.quantity);
  }
  return Object.entries(byProduct).map(([product_id, info]) => {
    ...
    return {
      ...
      unit_id: "",
      warehouse_id: info.warehouse_id ? String(info.warehouse_id) : "",
    };
  });
}

function InvoiceForm({ customers, warehouses, products, taxTypes, isAdmin, onCreated, invoice }) {
  const editing = !!invoice;
  // Determine which tax type IDs were applied to this invoice.
  const existingTaxIds = editing
    ? (invoice.tax_lines || []).map((tl) => String(tl.tax_type_id))
    : [];
  const [form, setForm] = useState(
    editing
      ...
      : {
          ...
          tax_type_ids: taxTypes?.filter((t) => t.is_active).map((t) => String(t.id)) ?? [],
          credit_override: false,
        }
  );
      ...
      lines.map((l, i) =>
        i === index
          ? {
              ...
              unit_id: "",
              warehouse_id: match?.default_warehouse_id ? String(match.default_warehouse_id) : "",
            }
          : l
      )
    );
  ...
  const submit = async (event) => {
    ...
    const payload = {
      ...form,
      tax_type_ids: form.tax_type_ids.map(Number),
      lines: lines
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="space-y-2">
        <label className="block text-sm font-bold text-slate-600">الضرائب المطبقة</label>
        <div className="flex flex-wrap gap-3">
          {(taxTypes || []).map((tt) => (
            <label key={tt.id} className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={form.tax_type_ids.includes(String(tt.id))}
                onChange={(e) => {
                  const id = String(tt.id);
                  const next = e.target.checked
                    ? [...form.tax_type_ids, id]
                    : form.tax_type_ids.filter((x) => x !== id);
                  setForm({ ...form, tax_type_ids: next });
                }}
                className="rounded border-gray-300"
              />
              {tt.name} ({(parseFloat(tt.rate) * 100).toFixed(1)}%)
            </label>
          ))}
          {(!taxTypes || taxTypes.length === 0) && (
            <span className="text-xs text-slate-400">لا توجد أنواع ضريبة نشطة</span>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        {lines.map((line, index) => {
          ...
          const warehouseName = line.warehouse_id
            ? warehouses?.find((w) => String(w.id) === line.warehouse_id)?.name ?? ""
            : "";
          return (
            <div key={index} className="mb-2 grid grid-cols-13 items-end gap-2" style={{ gridTemplateColumns: "5fr 1.5fr 2fr 2.5fr auto" }}>
              <div>
                <Input
              ...
              <div>
                <Input
                  label={index === 0 ? "المستودع" : undefined}
                  value={warehouseName}
                  readOnly
...
export default function SalesPage() {
  ...
  const taxTypes = useFetch(() => api.get("/tax-types/"));

  if (customers.loading || warehouses.loading || products.loading || taxTypes.loading) return <Loading />;
        ...
        <Card title="مرتجعات المبيعات">
          ...
          {returns.loading ? (
            ...
            <PaginatedTable
              ...
              dateFromField="created_at"
              dateToField="created_at"
            />
          )}
        </Card>
      )}
          ...
          <div className="space-y-4">
            <Table
              columns={[
                ...
                { key: "batch_number", label: "التشغيلية" },
                {
                  key: "warehouse_id",
                  label: "المستودع",
                  render: (r) => warehouses.data?.find((w) => w.id === r.warehouse_id)?.name ?? r.warehouse_id,
                },
