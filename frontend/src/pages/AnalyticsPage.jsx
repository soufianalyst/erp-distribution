...
import { Alert, Badge, Button, Card, Loading, PaginatedTable, Table, money, qty } from "../components/Ui";
import useFetch from "../hooks/useFetch";
import api from "../services/api";

const TIER_LABELS = { wholesale: "جملة", half_wholesale: "نصف جملة", retail: "تجزئة" };

/* ── Segment colours: vivid for chart fills, light-bg/dark-text for badges ── */
...
/* ── Shared chart typography (SKILL: ~13px, dark slate, semi-bold) ── */
const AXIS_TICK  = { fontSize: 13, fill: "#334155", fontWeight: 600 };
const AXIS_LABEL = { fontSize: 13, fill: "#0f172a", fontWeight: 700 };
const LEGEND_STYLE = { fontSize: 13, fontWeight: 700, color: "#334155" };
const DATA_LABEL  = { fontSize: 12, fontWeight: 700, fill: "#0f172a" };
const INSIDE_LABEL = { fontSize: 12, fontWeight: 700, fill: "#ffffff" };

/* ── Badge helper (light bg + dark text per SKILL) ── */
function segmentBadge(segment) {
  const cls = SEGMENT_BADGE_STYLES[segment] || "bg-slate-100 text-slate-700";
  return <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-bold ${cls}`}>{segment}</span>;
}

function Kpi({ label, value, tone = "slate", hint }) {
  const tones = {
    emerald: "text-emerald-700", rose: "text-rose-700", amber: "text-amber-700",
    sky: "text-sky-700", slate: "text-slate-800",
  };
  return (
...
export default function AnalyticsPage() {
  ...
  if (summary.loading || salesTrend.loading) return <Loading />;
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      {tab === "overview" && (
        <OverviewTab summary={summary.data} salesTrend={salesTrend.data || []} byWarehouse={byWarehouse.data || []} byPriceTier={byPriceTier.data || []} returnsTrend={returnsTrend.data || []} />
      )}
      {tab === "customers" && <CustomerRfmTab rows={customerRfm.data || []} loading={customerRfm.loading} />}
      {tab === "products"  && <ProductRfmTab rows={productRfm.data || []} loading={productRfm.loading} />}
      {tab === "inventory" && <InventoryTab expiryRisk={expiryRisk.data || []} turnover={turnover.data || []} loading={expiryRisk.loading || turnover.loading} />}
      {tab === "credit"    && <CreditTab aging={arAging.data || []} risk={creditRisk.data || []} loading={arAging.loading || creditRisk.loading} />}
      {tab === "delivery"  && <DeliveryTab fulfillment={fulfillment.data || []} drivers={drivers.data || []} loading={fulfillment.loading || drivers.loading} />}
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Kpi label="ذمم العملاء المستحقة" value={money(summary.ar_outstanding)} tone="amber" hint="إجمالي المبالغ المستحقة على جميع العملاء" />
        <Kpi label="قيمة مخزون معرضة للهدر" value={money(summary.waste_risk_value_30d)} tone="rose" hint="تشغيلات تنتهي صلاحيتها خلال 30 يوماً" />
        ...
        <div className="h-72 w-full" dir="ltr">
          <ResponsiveContainer>
            <AreaChart data={salesTrend} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
              ...
              <XAxis dataKey="period" reversed tick={AXIS_TICK} />
      ...
      {/* Warehouse + Price Tier side by side */}
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="h-56 w-full" dir="ltr">
            <ResponsiveContainer>
              <BarChart data={byWarehouse} layout="vertical" margin={{ top: 5, right: 15, left: 5, bottom: 5 }}>
                ...
                <YAxis dataKey="warehouse_name" type="category" width={110} tick={AXIS_TICK} interval={0} />
                <Tooltip formatter={(v) => money(v)} />
                <Bar dataKey="revenue" name="الإيرادات" fill="#0284c7">
                  <LabelList dataKey="revenue" position="insideRight" formatter={money} style={INSIDE_LABEL} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <Card title="الإيرادات حسب فئة السعر">
          <div className="h-56 w-full" dir="ltr">
            <ResponsiveContainer>
              <BarChart data={byPriceTier.map((r) => ({ ...r, tier_label: TIER_LABELS[r.price_tier] || r.price_tier }))} layout="vertical" margin={{ top: 5, right: 15, left: 5, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" tick={AXIS_TICK} />
                <YAxis dataKey="tier_label" type="category" width={110} tick={AXIS_TICK} interval={0} />
                <Tooltip formatter={(v) => money(v)} />
        ...
        <div className="h-64 w-full" dir="ltr">
          <ResponsiveContainer>
            <LineChart data={returnsTrend} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="period" reversed tick={AXIS_TICK} />
              <YAxis unit="%" tick={AXIS_TICK} width={50} />
              <Tooltip formatter={(v) => `${v}%`} />
              <Legend wrapperStyle={LEGEND_STYLE} />
              <Line type="monotone" dataKey="return_rate_pct" name="نسبة المرتجعات" stroke="#e11d48" strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="h-64 w-full" dir="ltr">
            <ResponsiveContainer>
              <BarChart data={segments} layout="vertical" margin={{ top: 5, right: 40, left: 5, bottom: 5 }}>
                ...
                <YAxis dataKey="segment" type="category" width={140} tick={AXIS_TICK} interval={0} />
                <Tooltip />
                <Bar dataKey="count" name="عدد العملاء">
                  <LabelList dataKey="count" position="right" style={DATA_LABEL} />
                  {segments.map((s, i) => <Cell key={i} fill={SEGMENT_COLORS[s.segment] || "#64748b"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        {/* Scatter: Recency × Frequency, bubble = monetary */}
        <Card title="الحداثة × التكرار (حجم الفقاعة = القيمة النقدية)">
          <div className="h-64 w-full" dir="ltr">
            <ResponsiveContainer>
              <ScatterChart margin={{ top: 10, right: 20, left: 10, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" dataKey="x" name="أيام منذ آخر شراء" tick={AXIS_TICK}>
                  <Label value="الحداثة (يوم)" position="insideBottom" offset={-10} style={AXIS_LABEL} />
                </XAxis>
                <YAxis type="number" dataKey="y" name="التكرار" tick={AXIS_TICK} width={45}>
                  <Label value="التكرار" angle={-90} position="insideLeft" style={AXIS_LABEL} />
                </YAxis>
                ...
                <Tooltip
                  ...
                  content={({ payload }) =>
                    payload?.[0] ? (
                      <div className="rounded-lg border border-slate-200 bg-white p-2.5 text-xs shadow-lg" dir="rtl">
                ...
                <Scatter data={scatterData}>
                  {scatterData.map((d, i) => <Cell key={i} fill={SEGMENT_COLORS[d.segment] || "#64748b"} />)}
                </Scatter>
              </ScatterChart>
            </ResponsiveContainer>
          </div>
      ...
      <Card title="تفاصيل تحليل RFM للعملاء">
        <PaginatedTable
          columns={[
            ...
            { key: "recency_days", label: "الحداثة", render: (r) => r.recency_days === null ? "—" : `${r.recency_days} يوم` },
          ...
          rows={rows} keyField="customer_id" empty="لا توجد بيانات."
          searchable searchPlaceholder="بحث بالعميل..."
        />
      </Card>
    </div>
  );
...
function ProductRfmTab({ rows, loading }) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="h-64 w-full" dir="ltr">
            <ResponsiveContainer>
              <BarChart data={rows.slice(0, 10)} layout="vertical" margin={{ top: 5, right: 60, left: 5, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" tick={AXIS_TICK} />
                <YAxis dataKey="product_name" type="category" width={130} tick={AXIS_TICK} interval={0} />
                <Tooltip formatter={(v) => money(v)} />
                <Bar dataKey="monetary" name="القيمة النقدية" fill="#059669">
                  <LabelList dataKey="monetary" position="right" formatter={money} style={DATA_LABEL} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      {/* Waste-risk highlight: dead stock near expiry */}
      {wasteRisk.length > 0 && (
        <Card title="⚠️ أولوية الهدر: أصناف راكدة قريبة من انتهاء الصلاحية">
          <PaginatedTable
            columns={[
              ...
              { key: "nearest_expiry_days", label: "أقرب انتهاء صلاحية", render: (r) => <Badge tone="red">{r.nearest_expiry_days} يوم</Badge> },
              { key: "recency_days", label: "آخر بيع", render: (r) => r.recency_days === null ? "لم يُبع" : `منذ ${r.recency_days} يوم` },
            ]}
            rows={wasteRisk} keyField="product_id" empty="—"
          />
        </Card>
      )}
      ...
      <Card title="تفاصيل تحليل RFM للأصناف">
        <PaginatedTable
          columns={[
            ...
            { key: "stock_on_hand", label: "المخزون الحالي", render: (r) => qty(r.stock_on_hand) },
            { key: "nearest_expiry_days", label: "أقرب انتهاء", render: (r) => r.nearest_expiry_days === null ? "—" : `${r.nearest_expiry_days} يوم` },
            { key: "segment", label: "الشريحة", render: (r) => segmentBadge(r.segment) },
          ]}
          rows={rows} keyField="product_id" empty="لا توجد بيانات."
          searchable searchPlaceholder="بحث بالصنف..."
        />
      </Card>
    </div>
  );
      ...
      <Card title="⚠️ تشغيلات قريبة من انتهاء الصلاحية (30 يوماً) — القيمة المعرضة للخطر">
        <PaginatedTable
          columns={[
            ...
            { key: "days_remaining", label: "الأيام المتبقية", render: (r) => <Badge tone={r.days_remaining <= 7 ? "red" : "amber"}>{r.days_remaining} يوم</Badge> },
            { key: "quantity", label: "الكمية", render: (r) => qty(r.quantity) },
            { key: "value_at_risk", label: "القيمة المعرضة للخطر", render: (r) => <b className="text-rose-700">{money(r.value_at_risk)}</b> },
          ]}
          rows={expiryRisk} keyField="batch_id" empty="لا توجد تشغيلات قريبة من الانتهاء — ممتاز!"
        />
      </Card>
        ...
        <div className="h-72 w-full" dir="ltr">
          <ResponsiveContainer>
            <BarChart data={turnover.slice(0, 15)} layout="vertical" margin={{ top: 5, right: 40, left: 5, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis type="number" tick={AXIS_TICK} />
              <YAxis dataKey="product_name" type="category" width={140} tick={AXIS_TICK} interval={0} />
              <Tooltip />
              <Bar dataKey="turnover_ratio" name="معدل الدوران" fill="#0284c7">
                <LabelList dataKey="turnover_ratio" position="right" style={DATA_LABEL} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
          ...
          <ResponsiveContainer>
            <BarChart data={agingTotals} margin={{ top: 20, right: 10, left: 10, bottom: 5 }}>
              ...
              <Bar dataKey="total" name="المبلغ المستحق">
                <LabelList dataKey="total" position="top" formatter={money} style={DATA_LABEL} />
                {agingTotals.map((_, i) => <Cell key={i} fill={["#059669", "#0284c7", "#d97706", "#e11d48"][i]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <Card title="العملاء الأعلى استغلالاً لحدهم الائتماني">
        <PaginatedTable
          columns={[
            ...
            { key: "utilization_pct", label: "نسبة الاستغلال", render: (r) => (
              <Badge tone={Number(r.utilization_pct) >= 90 ? "red" : Number(r.utilization_pct) >= 60 ? "amber" : "green"}>{r.utilization_pct}%</Badge>
            )},
            { key: "recency_days", label: "آخر شراء", render: (r) => r.recency_days === null ? "—" : `منذ ${r.recency_days} يوم` },
          ]}
          rows={risk} keyField="customer_id" empty="لا يوجد عملاء معرضين للخطر."
        />
      </Card>
      ...
      <Card title="تفاصيل أعمار الذمم حسب العميل">
        <PaginatedTable
          columns={[
            ...
            { key: "bucket_90_plus", label: "90+ يوم", render: (r) => <b className="text-rose-700">{money(r.bucket_90_plus)}</b> },
            { key: "total_outstanding", label: "الإجمالي", render: (r) => <b>{money(r.total_outstanding)}</b> },
          ]}
          rows={aging} keyField="customer_id" empty="لا توجد ذمم مستحقة."
        />
      </Card>
    </div>
  );
      ...
      <Card title="أداء السائقين">
        <PaginatedTable
          columns={[
            ...
            { key: "failure_rate_pct", label: "نسبة الفشل", render: (r) => (
              <Badge tone={Number(r.failure_rate_pct) >= 20 ? "red" : "green"}>{r.failure_rate_pct}%</Badge>
            )},
          ]}
          rows={drivers} keyField="driver_name" empty="لا توجد بيانات."
        />
      </Card>
    </div>
  );
        ...
        <div className="h-64 w-full" dir="ltr">
          <ResponsiveContainer>
            <BarChart data={rows} layout="vertical" margin={{ top: 5, right: 60, left: 5, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis type="number" tick={AXIS_TICK} />
              <YAxis dataKey="salesman_name" type="category" width={130} tick={AXIS_TICK} interval={0} />
              <Tooltip formatter={(v) => money(v)} />
              <Bar dataKey="revenue" name="الإيرادات" fill="#059669">
                <LabelList dataKey="revenue" position="right" formatter={money} style={DATA_LABEL} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <Card title="تفاصيل أداء المناديب">
        <PaginatedTable
          columns={[
            ...
            { key: "return_rate_pct", label: "نسبة المرتجعات", render: (r) => (
              <Badge tone={Number(r.return_rate_pct) >= 5 ? "red" : "green"}>{r.return_rate_pct}%</Badge>
            )},
          ]}
          rows={rows} keyField="salesman_id" empty="لا توجد بيانات."
        />
      </Card>
    </div>
  );
