...
class DeliveryService:
    ...
    async def list_invoice_summaries(self) -> list[DeliveryInvoiceSummary]:
        ...
        Only shows invoices ready for fulfillment:
        - Paid cash/card invoices (paid_amount >= total)
        - All credit invoices (handled in accounts, skip cashier)
        """
        result = await self.session.execute(
            select(SalesInvoice, Customer.name)
            .join(Customer, SalesInvoice.customer_id == Customer.id)
            .where(
                # Credit invoices go directly to delivery.
                (SalesInvoice.payment_method == SalesPaymentMethod.CREDIT)
                |
                # Cash/card invoices only when paid (cashier has collected).
                (
                    (SalesInvoice.payment_method.in_([SalesPaymentMethod.CASH]))
    ...
    async def invoice_prep(self, invoice_id: int) -> InvoicePrepOut:
        ...
        lines_result = await self.session.execute(
            ...
            .group_by(
                Product.name, SalesInvoiceLine.batch_number, Product.base_unit_name
            )
        ...
        lines = [
            ...
            for name, batch_number, quantity, unit in lines_result.all()
        ]
    ...
    async def add_invoice(self, trip_id: int, invoice_id: int) -> DeliveryTrip:
        ...
        if invoice is None:
            raise AppException(404, "فاتورة المبيعات غير موجودة.")
        # Multi-warehouse: check if the invoice has items from this trip's warehouse.
        line_warehouses = await self.session.execute(
            select(SalesInvoiceLine.warehouse_id)
            .where(SalesInvoiceLine.invoice_id == invoice_id)
            .distinct()
        )
        warehouses = {row[0] for row in line_warehouses.all()}
        if trip.warehouse_id not in warehouses:
            raise AppException(400, "لا توجد أصناف في هذه الفاتورة من مستودع الرحلة.")
        if invoice.fulfillment == FulfillmentType.PICKUP:
            raise AppException(
                400, "هذه الفاتورة استلام من المستودع ولا تُضاف لرحلات التوصيل."
            )
        # Cash invoices must be paid (through cashier) before delivery.
        if (
            invoice.payment_method != SalesPaymentMethod.CREDIT
            and invoice.paid_amount < invoice.total
        ):
            raise AppException(
                400, "هذه الفاتورة نقدية ولم تُسدد بعد؛ يجب تحصيلها عبر الصندوق أولاً."
            )
    ...
    async def picking_list(self, trip_id: int) -> PickingListOut:
        ...
        if invoice_ids:
            result = await self.session.execute(
                ...
                .order_by(SalesInvoiceLine.warehouse_id, Product.name, SalesInvoiceLine.batch_number)
            )
            for product_id, name, unit, batch_number, warehouse_id, quantity in result.all():
                quantity = Decimal(str(quantity))
                lines.append(
                    PickingLineOut(
