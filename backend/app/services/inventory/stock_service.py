"""Stock movements: receiving, FEFO allocation, transfers, adjustments, levels, and expiry alerts."""
...
class StockService:
    ...
    async def fefo_allocate_all(
        self, product_id: int, base_quantity: Decimal
    ) -> list[tuple[ProductBatch, Decimal]]:
        """FEFO across ALL warehouses. Batches carry their warehouse_id.

        Returns allocations just like fefo_allocate — callers own the transaction.
        """
        result = await self.session.execute(
            select(ProductBatch)
            .where(
    ...
    async def adjust_stock(
        ...
        """Create a stock adjustment: increase or decrease inventory."""
        await self.get_active_warehouse(data.warehouse_id)
        adjustment = StockAdjustment(
            warehouse_id=data.warehouse_id,
            adjustment_type=data.adjustment_type,
            reason=data.reason,
            adjustment_date=data.adjustment_date or date.today(),
        ...
        for line in data.lines:
            product = await self.get_active_product(line.product_id)
            line_total = (line.quantity * line.unit_cost).quantize(
                Decimal("0.01")
            )

            if data.adjustment_type == "increase":
                await self.add_stock_no_commit(
                    product_id=line.product_id,
                    warehouse_id=data.warehouse_id,
                    batch_number=f"ADJ-{adjustment.id}",
                ...
                allocations = await self.fefo_allocate(
                    line.product_id, data.warehouse_id, line.quantity
                )
