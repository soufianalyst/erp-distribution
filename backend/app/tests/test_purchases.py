...
class TestPurchaseOrders:
    async def _order(
        ...
        """Raise a draft order for `quantity`; returns (order, product, supplier_id, warehouse_id)."""
        warehouse_id, product, supplier_id = await setup_catalog(client, admin)
        response = await client.post(
            ...
            json={
                "supplier_id": supplier_id,
                "warehouse_id": warehouse_id,
                "expected_date": days_from_now(14),
                "lines": [
                    {
                        "product_id": product["id"],
                        "quantity": quantity,
        ...
        assert response.status_code == 201, response.text
        return response.json()["data"], product, supplier_id, warehouse_id

    @staticmethod
    def _receipt_line(order: dict, quantity: str, batch: str = "PO-B1") -> dict:
        return {
    ...
    async def test_draft_order_has_no_stock_or_accounting_effect(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, product, supplier_id, _ = await self._order(client, admin)

        assert order["status"] == "draft"
        assert as_decimal(order["subtotal"]) == Decimal("800.00")
        assert as_decimal(order["lines"][0]["received_quantity"]) == Decimal("0")
        assert as_decimal(order["lines"][0]["outstanding_quantity"]) == Decimal("100")
        assert order["received_invoice_ids"] == []

        # Nothing entered the warehouse and the supplier is owed nothing yet.
        batches = (
            await client.get(
                f"/api/v1/inventory/products/{product['id']}/batches", headers=admin
            )
    ...
    async def test_draft_cannot_be_received(self, client: AsyncClient) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, _, _, _ = await self._order(client, admin)
        ...
        response = await client.post(
            ...
            json={
                "payment_method": "credit",
                "lines": [self._receipt_line(order, "100")],
            },
        )
        assert response.status_code == 400
        assert "أرسل الطلب" in response.json()["message"]

    async def test_send_then_full_receipt_closes_order(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, product, supplier_id, _ = await self._order(client, admin)
        ...
        sent = await client.post(
            f"/api/v1/purchases/orders/{order['id']}/send", headers=admin
        )
        ...
        invoice = received.json()["data"]
        assert as_decimal(invoice["subtotal"]) == Decimal("800.00")
        assert as_decimal(invoice["vat_amount"]) == Decimal("128.00")
        assert as_decimal(invoice["total"]) == Decimal("928.00")

        # The order is closed and points at the delivery that fulfilled it.
        after = (
            await client.get(f"/api/v1/purchases/orders/{order['id']}", headers=admin)
        ).json()["data"]
        assert after["status"] == "received"
        assert as_decimal(after["lines"][0]["received_quantity"]) == Decimal("100")
        assert as_decimal(after["lines"][0]["outstanding_quantity"]) == Decimal("0")
        assert after["received_invoice_ids"] == [invoice["id"]]

        # The goods really landed, at the ordered cost.
        batches = (
            await client.get(
                f"/api/v1/inventory/products/{product['id']}/batches", headers=admin
            )
        ...
        assert len(batches) == 1
        assert batches[0]["batch_number"] == "PO-B1"
        assert as_decimal(batches[0]["quantity"]) == Decimal("100")
        assert as_decimal(batches[0]["unit_cost"]) == Decimal("8.00")

    async def test_partial_receipt_leaves_order_open(self, client: AsyncClient) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, product, _, _ = await self._order(client, admin)
        await client.post(f"/api/v1/purchases/orders/{order['id']}/send", headers=admin)
        ...
        first = await client.post(
            ...
            json={
                "payment_method": "credit",
                "lines": [self._receipt_line(order, "40", "PO-B1")],
            },
        )
        ...
        assert as_decimal(mid["lines"][0]["received_quantity"]) == Decimal("40")
        assert as_decimal(mid["lines"][0]["outstanding_quantity"]) == Decimal("60")

        # The rest arrives later as its own batch and its own invoice.
        second = await client.post(
            f"/api/v1/purchases/orders/{order['id']}/receive",
            headers=admin,
            json={
                "payment_method": "credit",
                "lines": [self._receipt_line(order, "60", "PO-B2")],
            },
        )
        ...
        assert as_decimal(final["lines"][0]["outstanding_quantity"]) == Decimal("0")
        assert len(final["received_invoice_ids"]) == 2
        ...
        assert {b["batch_number"] for b in batches} == {"PO-B1", "PO-B2"}
        assert sum(as_decimal(b["quantity"]) for b in batches) == Decimal("100")

    async def test_receipt_beyond_outstanding_rejected(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, product, _, _ = await self._order(client, admin)
        ...
        response = await client.post(
            ...
            json={
                "payment_method": "credit",
                "lines": [self._receipt_line(order, "101")],
            },
        )
        assert response.status_code == 400
        assert "أكبر من المتبقي" in response.json()["message"]

        # Rejected before anything was applied: no stock, order still fully open.
        batches = (
            await client.get(
                f"/api/v1/inventory/products/{product['id']}/batches", headers=admin
            )
        ...
        assert as_decimal(after["lines"][0]["received_quantity"]) == Decimal("0")

    async def test_receipt_can_override_the_ordered_cost(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, product, _, _ = await self._order(client, admin)
        ...
        line = self._receipt_line(order, "100") | {"unit_cost": "9.50"}
        response = await client.post(
            f"/api/v1/purchases/orders/{order['id']}/receive",
            headers=admin,
            json={"payment_method": "credit", "lines": [line]},
        )
        assert response.status_code == 201, response.text
        # The invoice records what was actually paid, not what was expected.
        assert as_decimal(response.json()["data"]["subtotal"]) == Decimal("950.00")
        # The order keeps its own expected value for comparison.
        after = (
            await client.get(f"/api/v1/purchases/orders/{order['id']}", headers=admin)
        ).json()["data"]
        assert as_decimal(after["subtotal"]) == Decimal("800.00")

    async def test_cancelled_order_cannot_be_received(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, _, _, _ = await self._order(client, admin)
        ...
        cancelled = await client.post(
            f"/api/v1/purchases/orders/{order['id']}/cancel",
            headers=admin,
            json={"cancel_reason": "المورد لم يستطع التوريد"},
        )
        ...
        assert response.status_code == 400
        assert "ملغى" in response.json()["message"]

    async def test_fully_received_order_cannot_be_cancelled(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, _, _, _ = await self._order(client, admin)
        ...
        response = await client.post(
            f"/api/v1/purchases/orders/{order['id']}/cancel", headers=admin, json={}
        )
        assert response.status_code == 400

    async def test_partially_received_order_can_be_cancelled(
        self, client: AsyncClient
    ) -> None:
        """Cancelling here abandons the remainder; what arrived stays in stock."""
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, product, _, _ = await self._order(client, admin)
        await client.post(f"/api/v1/purchases/orders/{order['id']}/send", headers=admin)
        await client.post(
            ...
            json={
                "payment_method": "credit",
                "lines": [self._receipt_line(order, "40")],
            },
        )
        ...
        response = await client.post(
            f"/api/v1/purchases/orders/{order['id']}/cancel",
            headers=admin,
            json={"cancel_reason": "توقف التعامل مع المورد"},
        )
        assert response.status_code == 200, response.text
        ...
        assert as_decimal(batches[0]["quantity"]) == Decimal("40")

    async def test_draft_can_be_edited_but_sent_order_cannot(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, product, supplier_id, warehouse_id = await self._order(client, admin)

        body = {
            "supplier_id": supplier_id,
            "warehouse_id": warehouse_id,
            "lines": [
                {"product_id": product["id"], "quantity": "50", "unit_cost": "7.00"}
            ],
        }
        edited = await client.put(
            f"/api/v1/purchases/orders/{order['id']}", headers=admin, json=body
        )
        assert edited.status_code == 200, edited.text
        assert as_decimal(edited.json()["data"]["subtotal"]) == Decimal("350.00")
        assert len(edited.json()["data"]["lines"]) == 1
        ...
        assert "مسودة" in blocked.json()["message"]

    async def test_receipt_line_from_another_order_rejected(
        self, client: AsyncClient
    ) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, _, _, _ = await self._order(client, admin)
        ...
        response = await client.post(
            ...
            json={
                ...
                "lines": [
                    {
                        "order_line_id": order["lines"][0]["id"] + 9999,
                        "quantity": "1",
                        "batch_number": "PO-X",
                        "expiry_date": days_from_now(120),
                    }
    ...
    async def test_orders_can_be_filtered_by_status(self, client: AsyncClient) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        order, _, _, _ = await self._order(client, admin)
        await client.post(f"/api/v1/purchases/orders/{order['id']}/send", headers=admin)
        ...
        assert order["id"] not in [o["id"] for o in drafts]
        ...
        assert order["id"] in [o["id"] for o in sent]

    async def test_sales_role_cannot_create_order(self, client: AsyncClient) -> None:
        admin = await login(client, "admin", TEST_ADMIN_PASSWORD)
        warehouse_id, product, supplier_id = await setup_catalog(client, admin)
